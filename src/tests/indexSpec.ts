...
describe('Test to verify endpoint and resize image', () => {
  it('GET /api/images should return a resized image', async () => {
    const response = await request(app).get('/api/images?filename=palmtunnel&width=100&height=100').expect(200);
...
describe('Image Cache Tests', () => {
  it('should return the same cached image for identical resizing parameters', async () => {
    ...
    const response1 = await request(app).get('/api/images?filename=palmtunnel&width=200&height=300').expect(200);

    const response2 = await request(app).get('/api/images?filename=palmtunnel&width=200&height=300').expect(200);
  ...
  it('should return different images for different resizing parameters', async () => {
    ...
    const response2 = await request(app).get('/api/images?filename=palmtunnel&width=400&height=600').expect(200);
